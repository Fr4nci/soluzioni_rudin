...
\end{myproof}
\thm{Proprietà della chiusura di un insieme}{Siano $E$ e $F$ due insiemi, allora $\overline{E \cup F} = \bar{E} \cup \bar{F}$}
\begin{myproof}
...
\end{enumerate}
\begin{mysolution}
	per mostrare $(a)$ bisogna mostrare che $\bar{B_n} \subseteq \bigcup\limits_{i=1}^n \bar{A_i}$ e $\bigcup\limits_{i=1}^n \bar{A_i} \subseteq \bar{B_n}$. Si osserva che se $x \in \bar{B_n}$ implica che $x \in B_n \cup (B_n)'$ 
\end{mysolution}
\end{document}