...
\noindent e si osserva che $\forall b^s \in B(x)$ e $\forall b^t \in B(y) \implies b^{s+t} \in B(x+y)$ siccome $s \leq x$ e $t \leq y$ dunque $s+t \leq x+y$ dunque $Z \subset B(x+y)$ con $Z = \{ b^z \in \mathbb{R}: \exists s \leq x, t \leq y: b^z = b^s b^t \}$ dunque $\sup{B(x)B(y)} \leq \sup{B(x+y)}$. Tuttavia si osserva che $\forall z \in \mathbb{R} : b^z \in B(x+y)$ possiamo considerare invece i numeri razionali che soddisfano la seguente proprietà $t-x < p < y$ e consideriamo a questo punto $q = t - p$ da cui avremo che $t-x < p \implies t - p < x \implies q < x$ ma allora $t = p + q$, dunque:
$$
b^t = b^{p+q} \stackrel{\text{per quanto visto sopra}}{=} b^p b^q
$$
e da qua possiamo tranquillamente considerare $x > p$ e $y> q$ per far vedere che $\forall z: b^z \in B(x+y) \exists s, t: b^s b^t \in B(x)B(y) : b^z < b^s b^t$ dunque $\sup{B(x+y)} \leq \sup{B(x)B(y)}$
\end{myproof}
\end{document}